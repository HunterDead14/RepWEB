...
const { data: categoryData } = await useFetch<any>(`http://127.0.0.1:8000/api/admin/blog/categories/${categoryId}`)
...
const onSubmit = async () => {
  pending.value = true
  try {
    await $fetch(`http://127.0.0.1:8000/api/admin/blog/categories/${categoryId}`, {
      method: 'PATCH',
      body: {
        title: state.title,
